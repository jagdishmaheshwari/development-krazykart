...
<head>
    ...
    <title>@yield('title', 'Krazy Kart')</title>
    ...
    <style>
        ...
        .cube .face:nth-child(1) {
            ...
            background-image: url(/loader/loading-1.jpg);
        }

        .cube .face:nth-child(2) {
            transform: rotateX(90deg) translateZ(75px);
            background-image: url(/loader/loading-2.jpg);
        }

        .cube .face:nth-child(3) {
            transform: rotateX(180deg) translateZ(75px) rotateZ(90deg);
            background-image: url(/loader/loading-4.jpg);
        }

        .cube .face:nth-child(4) {
            transform: rotateX(-90deg) translateZ(75px) rotateZ(90deg);
            background-image: url(/loader/loading-5.jpg);
        }

        .cube .face:nth-child(5) {
            transform: rotateY(-90deg) translateZ(75px) rotateZ(-90deg);
            background-image: url(/loader/loading-3.jpg);
        }

        .cube .face:nth-child(6) {
            transform: rotateY(90deg) translateZ(75px);
            background-image: url(/loader/loading-6.jpg);
        }
    ...
    <div id="loading-overlay">
        <div class="cube-container">
            <div class="cube"><i class="face"></i><i class="face"></i><i class="face"></i><i
                    class="face"></i><i class="face"></i><i class="face"></i></div>
        </div>
    </div>
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            hideLoader()
            $('.addToCart[data-product]').click(function() {
                ...
                // Send AJAX request
                $.ajax({
                    url: '{{ route('wishkart.add') }}',
                    type: 'POST',
                    data: data,
                    success: function(response) {
                        swal({
                            text: 'Added To Wishkart Successfully!',
                            icon: 'success'
                        })
                    },
                    error: function(xhr, status, error) {
                        // Handle error if needed
                        console.error('Error adding product to Wishkart:', error);
                    }
                });
            })
        });
        ...
        function hideLoader() {
            var e = document.getElementById("loading-overlay");
...
-->
    {{-- Current Language: {{ config('languages.' . App::getLocale()) }} --}}
    <?php $ActivePage = ''; ?>
